...
class DataProcessor:
    ...
    def load_data(self):
        ...
        self.data = xp.array(fid["data"],dtype = xp.float32) 
        ...
        self.translations_x = xp.array(fid["translations_x"]) #load the real and imag of dtype int64
        self.translations_y = xp.array(fid["translations_y"])

        # get the image extent (with wrap-around)
        self.Nx = int(xp.ceil(xp.max(self.translations_x) - xp.min(self.translations_x)))
        self.Ny = self.Nx
        fid.close()
#################################################

def Get_Mappings(processor,sync):
    #Split, Overlap = Split_Overlap_plan(processor.translations_x, processor.translations_y, processor.nx,processor.ny, processor.Nx,processor.Ny)
    if sync == True:
        if GPU:
           #calculate the preconditioner here
            Gplan = Gramiam_plan(processor.translations_x,processor.translations_y,processor.nframes,processor.nx,processor.ny,processor.Nx,processor.Ny, bw = 0)
