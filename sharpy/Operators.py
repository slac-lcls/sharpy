...
_eig_last_lam = None                  # lets warm calls enter graph mode at it=0
...
if GPU:
    ...
    extern "C" __global__ void eig_step(
        ...
        for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += gs) {
            ...
            l1 += (double)(yr*yr + yi*yi);
            l2 += (double)(yr*vr + yi*vi);
            l3 += (double)(vr*vr + vi*vi);
            float tr = yr, ti = yi;
            if (beta != 0.0f) {
                tr -= beta * vp[2*i]; ti -= beta * vp[2*i+1];
                y[2*i] = tr; y[2*i+1] = ti;
            }
            l0 += (double)(tr*tr + ti*ti);
        }
        int tid = threadIdx.x;
...
class _EigAdapt:
    ...
    def feed(self, rows):
        """rows: iterable of (ssq_t, ssq_y, dot). Updates self.beta/self.stop;
        beta takes effect from the NEXT kernel launch (batch-boundary delay is
        part of the contract on both backends)."""
        for ssq_t, ssq_y, dot, ssq_v in rows:
            if self.stop:
                return
            it = self.it
            self.it += 1
            if ssq_y <= 0.0 or ssq_v <= 0.0:
                self.stop = True            # H v == 0: nothing to iterate on
                return
            ...
            if self.beta == 0.0:
                ...
                if self.prev_step is not None and step < self.prev_step:
                    ...
                    if self.tol > 0 and step * r / (1.0 - r) < self.tol:
                        ...
                        return
                self.prev_step = step
                if self.momentum and it + 1 >= self.warmup \
                ...
                if it - self.last_update >= 2 * R \
                    ...
                    if 0.0 < sig < 1.0 and lam * lam > 4.0 * beta:
                        ...
                        nb = (lam2 / 2.0) ** 2
                        if nb > beta * 1.0001:      # climb-only; fixed point
                            self.beta = nb          #   at Chebyshev-optimal
                            self.last_update = it
...
def _eig_graph_get(H, n, blocks, threads, beta_dev):
    """Cached _EigGraph keyed on H's identity AND buffer pointers (val2H
    rewrites H.data in place on a persistent H, so hits are the norm; a
    reassembled H misses and fails closed to a fresh capture)."""
    key = (id(H), H.data.data.ptr, H.indices.data.ptr, H.indptr.data.ptr, n)
...
def _eig_power_fused(H, v, num_iter, tol):
    ...
    (K=_EIG_GRAPH_K) as ONE launch with prebuilt cuSPARSE descriptors."""
    global _eig_last_lam
    n = int(v.shape[0])
    ...
    # warm calls may enter graph mode at it=0 using the previous call's lam
    win = _EIG_WINDOWED and _win_ok(_eig_last_lam, max(K, Kg))
    gexec = None
    ...
    if ctl.lam is not None:
        _eig_last_lam = ctl.lam
