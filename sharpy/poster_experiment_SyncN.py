...
for sync_interval in xp.arange(1,15,4):
    ...
    img_sync.append(img1)
    residuals_sync.append(residuals_AP1)
    
    print("Sync total time:", sync_interval ,timer() - t0)
...
for i in range(len(nn)):
    axs[0].semilogy((residuals_sync[i][:, 0]).get(), label = {'sync_interval',round(nn[i])})
    axs[0].set_title("|img-f truth| (f=phase scalar)")
    
    axs[1].semilogy((residuals_sync[i][:, 1]).get(), label = {'sync_interval',round(nn[i])})
    axs[1].set_title("||frames|-data|")
    
    axs[2].semilogy((residuals_sync[i][:, 2]).get(), label = {'sync_interval',round(nn[i])})
